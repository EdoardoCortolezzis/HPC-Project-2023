#!/bin/bash
#SBATCH --job-name=OMP-WEAK-BORDERS
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
OUTPUT_FILE="../results/omp-weak/omp_weak_borders.csv"
echo "cores,threads,width,height,time" > "${OUTPUT_FILE}"

# image size grows with √threads
X_LEFT=-2.0
Y_LOWER=-1.0
X_RIGHT=1.0
Y_UPPER=1.0
...
for THREADS in {1..128}; do
    ...
    echo "OMP-WEAK-BORDERS: cores=1, threads=${THREADS}, width=${n}, height=${n}, time=${EXEC_TIME}"
done