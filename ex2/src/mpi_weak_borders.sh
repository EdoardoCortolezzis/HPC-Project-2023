#!/bin/bash
#SBATCH --job-name=MPI-WEAK-BORDERS
#SBATCH --nodes=2
...
OUTPUT_FILE="../results/mpi-weak/mpi_weak_borders.csv"
...
for CORES in {1..256}; do
    n=$(echo "scale=0; sqrt($CORES * $C)/1" | bc)
    EXEC_TIME=$(mpirun -np "${CORES}" --map-by core --bind-to core \
                ./mandel_borders_mpi \
                "${n}" "${n}" "${X_LEFT}" "${Y_LOWER}" \
                "${X_RIGHT}" "${Y_UPPER}" "${MAX_ITERATIONS}" 1)

    echo "${CORES},1,${n},${n},${EXEC_TIME}" >> "${OUTPUT_FILE}"
    echo "MPI-WEAK-BORDERS: cores=${CORES}, width=${n}, time=${EXEC_TIME}"
done